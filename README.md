...
**For full methodological details, please refer to our publication: https://www.frontiersin.org/journals/genetics/articles/10.3389/fgene.2026.1782221/full**

## Analysis Overview
This analysis pipeline consists of 11 main steps, including one optional step for AutoZI parameter tuning (step 4) and a check for ambient RNA inflluence to assess conclusions (step 11). We also include an additional optional step to compare novel isoforms against existing datasets that detected unannotated isoforms, which can be found in the /compare_annotations directory. 
...
### Step 11 -- SoupX Analyses
This step models ambient RNA correction using SoupX, and is performed with the raw filtered dataset from Step 2. "Empty" droplets (defined in our analyses as droplets with 100-500 reads) inform the ambient "soup" profile and estimates a per-cell contamination fraction (ρ ≈ 1%). This step also tests whether corrected matrices express GZMB and CD3G isoforms and reports the outcome by cluster, as seen in Supplemental Table 5. Because the contamination fraction was so low, these corrected matrices were not carried forward for AutoZI modeling.
